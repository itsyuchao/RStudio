...
* Using a `.csv` file, `forWordSeparation_cleaned`, to create a *word* by *participant* matrix with behavioral ratings (*emotional intensity* or *literariness*) from Qualtrics survey outputs. 
...
* By story 
* To check if *emotional intensity* and *literariness* are distinct and can serve as regressors in separate fMRI GLMs.
...
```{r All Emo~Lit} 
allEmoWordRating <- unlist(subset(allRatingMarked, Rating == "Emotional Intensity", select = "MN"))
allLitWordRating <- unlist(subset(allRatingMarked, Rating == "Literariness", select = "MN"))
allEmoEventRating <- unlist(subset(allRatingMarked, Rating == "Emotional Intensity", select = "MN_event"))
...
#set.seed(20) 
#randomLitRating <- runif(2344, 1, 2)
#cor.test(allEmoWordRating, randomLitRating, method = "pearson")
```
