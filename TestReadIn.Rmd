...
#MATCH EMO RESULTS    
emoResult <- read.csv(file="Verhalen_Emotional+Arousal_September+17%2C+2019_12.42.csv", 
...
#MATCH LIT RESULTS
litResult <- read.csv(file="Verhalen_Literariness_September+17%2C+2019_12.43.csv", 
                  header = TRUE, sep = ",", stringsAsFactors = FALSE)
...
limits <- aes(ymax = emoTrend$mean + emoTrend$se,
              ymin = emoTrend$mean - emoTrend$se)

ggplot(data = emoTrend, aes(x = vars, y = mean, color = vars)) + 
  geom_point(size=2.5, alpha=0.6, color = "orange") +
  geom_errorbar(limits, position = dodge, width = 0.25) +
  theme(axis.text.x=element_blank(), axis.ticks.x=element_blank(),
        axis.title.x=element_blank()) + 
  ggtitle("Emotion Rating (De Muur + De Hond concatenated), N=20")
...
ggplot(data = litTrend, aes(x = vars, y = mean, color = vars)) + 
  ...
  theme(axis.text.x=element_blank(), axis.ticks.x=element_blank(),
        axis.title.x=element_blank()) + 
  ggtitle("Literariness Rating (De Muur + De Hond concatenated), N=21")
